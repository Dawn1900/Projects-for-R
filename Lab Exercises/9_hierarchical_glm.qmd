...
parameters {
  ...
  real<lower=0> sigma_alpha;  // hyperparameter for the standard deviation of the intercepts
  real beta;  // covariate coefficient
}
model {
  vector[N] log_lambda = alpha + beta * x + offset;  // linear predictor
...
# Calculate the linear predictor for each region using the mean estimates of alpha and beta
...
# Extract the posterior samples for alpha and beta
posterior_alpha <- rstan::extract(different_intercept_model)$alpha  # This will be a matrix with dimensions iter x N
posterior_beta <- rstan::extract(different_intercept_model)$beta  # This will be a vector with length iter

# Calculate the mean of the posterior samples for alpha and beta
alpha_mean2 <- apply(posterior_alpha, 2, mean)  # Take the mean across iterations for each region
...
posterior_alpha3 <- rstan::extract(hierarchically_intercept_model)$alpha  # This will be a matrix: iter x N
posterior_mu_alpha <- rstan::extract(hierarchically_intercept_model)$mu_alpha  # This will be a vector: iter
posterior_sigma_alpha <- rstan::extract(hierarchically_intercept_model)$sigma_alpha  # This will be a vector: iter
posterior_beta3 <- rstan::extract(hierarchically_intercept_model)$beta  # This will be a vector: iter

# Calculate the mean of the posterior samples for alpha and beta
alpha_mean3 <- apply(posterior_alpha3, 2, mean)  # Take the mean across iterations for each region
...
### Differences in estimated $\theta_i$'s across regions
```{r}
library(ggplot2)
# Create data frames for each model
theta_data_common <- data.frame(region = 1:N, theta = theta_estimated, Model = "Common Intercept")
theta_data_separate <- data.frame(region = 1:N, theta = theta_estimated2, Model = "Separate Intercepts")
theta_data_hierarchical <- data.frame(region = 1:N, theta = theta_estimated3, Model = "Hierarchical Intercepts")
...
```
### Differences in estimated $\theta_i$'s across models
...
comparison_data <- data.frame(
  ...
  model = factor(rep(c("Model 1: Common Intercept", "Model 2: Separate Intercepts", "Model 3: Hierarchical"), each = N))
)
